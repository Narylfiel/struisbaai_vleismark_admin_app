  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final row = await _client.from('hunter_jobs').insert({
        ...
        'client_contact': _phoneCtrl.text.trim(),
    ...
    pdf.addPage(
      pw.MultiPage(
        build: (context) => [
          ...
          pw.Text('Hunter: ${_savedJob!['client_name'] ?? '—'}'),
          pw.Text('Phone: ${_savedJob!['client_contact'] ?? '—'}'),
          pw.Text('Date: ${_savedJob!['job_date'] ?? '—'}'),
          pw.Text('Estimated weight: ${_savedJob!['estimated_weight_kg']} kg'),
          pw.Text('Charge: R ${( _savedJob!['quoted_price'] as num?)?.toStringAsFixed(2) ?? '0.00'}'),
          pw.SizedBox(height: 8),
